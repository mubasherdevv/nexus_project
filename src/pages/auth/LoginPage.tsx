...
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result && result.twoFactorRequired) {
        ...
        setTwoFactorUserId(result.userId || null);
        setIsLoading(false);
        return;
      }
  ...
  const handleVerify2FA = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      if (!twoFactorUserId) throw new Error('Missing ID');
      await verify2FA(twoFactorUserId, otp);
      navigate(role === 'entrepreneur' ? '/dashboard/entrepreneur' : '/dashboard/investor');
    } catch (err) {
  ...
  const fillDemoCredentials = (userRole: UserRole) => {
    ...
    if (userRole === 'entrepreneur') {
      setEmail('mubasher12@gmail.com');
      setPassword('mubasher12');
    } else {
      setEmail('talha@gmail.com');
      setPassword('talha');
    }
    setRole(userRole);
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">
          {is2FARequired ? 'Security Verification' : 'Sign in to Business Nexus'}
        </h2>
        <p className="mt-2 text-center text-sm text-gray-600">
          {is2FARequired ? 'Please enter the 6-digit code sent to your device' : 'Connect with investors and entrepreneurs'}
        </p>
      </div>
